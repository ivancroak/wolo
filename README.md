...
**A full-stack, non-custodial marketplace for X (Twitter) content services, settled by Solana escrow and on-chain reputation.**
...
Wolo connects brands and individuals who need X content with the creators who produce it. Every order is backed by **native SOL escrow on Solana** — funds are locked by an on-chain program, not held in a platform wallet, and are released only by the program's rules (delivery, milestone approval, scheduled payroll, or arbiter-resolved dispute). Identity is anchored to a **cryptographically verified X handle**, trust is tracked by an **on-chain reputation program**, and discovery is driven by an **LLM marketplace assistant**.

This repository is a single engineer's end-to-end build: UI/UX, front-end, typed API layer, relational data model, two Rust/Anchor programs, the security stack, and the serverless automation that drives settlement.
...
Wolo is split across three runtimes that settle against one another: the **browser** (where users sign their own transactions), the **Next.js server** (typed API + Postgres + scheduled jobs), and the **Solana programs** (custody + reputation). The platform never takes custody of user funds — it orchestrates, but the on-chain program holds and releases value.
...
- **18+ tables** — `users`, `profiles`, `services`, `orders`, `escrows`, `milestones`, `payroll_periods`, `secure_messages`, `reputations`, `ratings`, `deal_proposals`, `notifications`, `sessions`, `watchlist`, and more — with extensive foreign keys (`on delete cascade`), `CHECK` constraints enforcing state enums, indexes on every hot column, and **partial-unique** constraints (e.g. one active order per service/buyer, one pending proposal per order).
...
- **TanStack React Query** for all data fetching and cache invalidation.
...
Notifications are written in-app and optionally delivered by **Resend** email when the recipient has a verified address.
...
- **Non-custodial escrow** — value lives in a program-owned PDA; releases follow on-chain rules, and disputes are settled by a designated arbiter key (not by moving funds to a platform wallet).
...
| **Identity** | Sign-in-with-Solana (Ed25519 / `tweetnacl`), twitterapi.io X-handle oracle |
| **Email** | Resend |
| **Infra** | Vercel (hosting + Cron), security headers, in-memory rate limiting |
...
src/
  ...
  lib/              Solana clients (hand-built instructions), Groq agent, utils
...
Wolo is a **working full-stack application deployed on Solana devnet**, presented here as an engineering portfolio project. A few honest notes so the code matches the claims:

- On-chain programs target **devnet**.
- **Automated test suites are not yet included** in this repository.
- Per-order **messaging currently uses short-interval polling**, not websocket push; the `secure_messages` schema is provisioned for end-to-end encryption, but encryption is **not yet wired** (messages are stored in plaintext for now).
